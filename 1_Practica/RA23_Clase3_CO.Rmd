...
## [Modelo aditivo]{style="color:darkred"}

<br>

### Ejemplo de pesos de recién nacidos

<br> Analizamos los siguientes datos que corresponden a mediciones de 100 niños nacidos con bajo peso (es decir, con menos de 1500 g.) en Boston, Massachusetts.
...
```{r, echo=TRUE}
file <- "C:/Austral/mcd-reg-adv/datasets/low_birth_weight_infants.txt"
...
<br> Ajustamos el perímetro cefálico, mediante un modelo lineal, a las covariables de edad gestacional y de peso al nacer.
...
<br> Generamos nuevos valores de las variables y obtenemos la predicción del perímetro cefálico para esos nuevos datos.
...
## [Modelos lineales robustos]{style="color:darkred"}
...
## [Análisis de multicolinealidad]{style="color:darkred"}
...
```{r, echo=TRUE}
library(readxl)
IMCinfantil<-read_excel("C:/Austral/mcd-reg-adv/datasets/IMCinfantil.xlsx")
...
Probamos eliminar la variable que tiene VIF superior (PESO), recordar además que PESO y CC tienen alta correlación=0.92, conviene más aún sacar entonces PESO.
...
## [Regresión polinómica]{style="color:darkred"}
...
## [Selección de variables]{style="color:darkred"}
...
## [Validación cruzada]{style="color:darkred"}
...
Se identifica el mejor modelo formado por 2 predictores empleando todas las observaciones (training + test).
...
## [CV usando paquete Caret]{style="color:darkred"}
